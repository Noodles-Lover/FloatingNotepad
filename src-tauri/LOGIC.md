...
每个整点弹出小窗（104×52）显示当前时刻，停留 3 秒后自己收起。

- **收起由 `show()` 自己排定**：整点报时与手动「试一下报时」是同一条路径，弹完都是 3 秒后收，不存在"手动弹的不会消失"。定时器带序号（`show_seq`），期间若又弹一次，旧定时器自动作废，不会把新窗提前收走。

- **独立窗口** `chime`：主窗口可能被穿透或隐藏，报时不能依赖它。setup 阶段预创建并常驻隐藏——它靠事件刷新内容，若等首次报时才创建，前端还没挂载，第一次事件就丢了。
- **整点判定**：复用 `db::local_clock()` 的「自当日 4 点起已过毫秒数」，加 4 小时对一小时取余即得距下个整点的时间；末尾多等 250ms，避免本地时钟只精确到秒而提前弹窗。
- **不透明度走 CSS，不用分层窗口**：曾用 Win32 分层窗口（`WS_EX_LAYERED` + `SetLayeredWindowAttributes`）实现，但给 WebView2 的宿主窗口加分层样式会让**内容整个不显示**——透明无边框的窗口看起来就是「什么都没弹出来」。现在由 Rust 把不透明度放进 `chime-show` 载荷，前端用 `style={{ opacity }}` 实现。
- **不抢焦点**：与解锁锁一致，`SW_SHOWNA` + `WS_EX_NOACTIVATE`；显示后再补一次不激活置顶，避免被后来者盖住。
- **新窗口必须登记能力**：`capabilities/default.json` 的 `windows` 数组决定了哪些窗口拥有这套权限。漏登记的窗口**一条权限都没有**——连 `core:event:allow-listen` 也被拒，于是收不到 `chime-show`，小窗渲染成一片空白（而音效由主窗口播放，听起来像"响了但没弹窗"）。新增窗口时务必把 label 加进 `windows`，并把新命令加进 `permissions/commands.toml` + `default.json`。
- **内容不依赖事件**：`chime-show` 在**显示之后**发（载荷含时刻与不透明度）；小窗挂载时还会主动取一次 `chime_state`，万一事件错过也不会空白或停在启动时刻。
- **音效不在报时窗播**：该窗口从没被用户点过，Chromium 会拦掉无用户交互的 audio；因此 `chime-show` 广播给所有窗口，由主窗口（已有用户交互）播放。
- 命令：`set_chime`（开关 + 不透明度）、`ring_chime`（立刻弹一次，供挂件右键菜单的「试一下报时」使用——面板打开时看不见挂件，测试入口只能放在挂件上）、`chime_state`（小窗挂载时取当前内容）。

## 9. 系统覆盖层排除（is_shell_overlay）
