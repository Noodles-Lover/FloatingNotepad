...
| 形态 | 含义 | 进入方式 |
| --- | --- | --- |
| `hidden` | 挂件贴边收窄，只露一条边（待命） | 启动 / 鼠标离开超时 / 手动关闭 / 托盘隐藏 / 穿透 |
| `revealed` | 挂件滑出完整可见 | 鼠标接近贴边区 / 拖动松手 |
| `expanded` | 展开为速记面板 | 点击挂件 |

此外还有「覆盖层面板」（皮肤 / 设置 / 使用统计 / 功能 / 日程）与「整窗隐藏」（托盘 / 右键）两项状态。

### 唯一入口：applyView

形态变化**只能**经 `App.tsx` 的 `applyView(target, opts)`，不允许在别处直接 `setMode` / 操作窗口。覆盖层只应存在于面板中：目标形态不是 `expanded` 时必须先关掉全部覆盖层、再改变窗口，否则面板会被压缩进挂件尺寸的窗口。

不变量在 `applyView` 内统一强制，任何调用方都绕不过：

| 编号 | 不变量 |
| --- | --- |
| I1 | 覆盖层只在面板里存在：目标形态不是 `expanded` 时无条件关闭全部覆盖层，**且在动窗口之前关** |
| I2 | 整窗隐藏 ⟹ 形态为 `hidden` |
| I3 | 穿透中 ⟹ 形态为 `hidden`（穿透时窗口点击穿透，面板既无意义也点不到） |

它与窗口形态一一对应：`expanded` → 面板尺寸 + 展开定位；`hidden` / `revealed` → 停靠回挂件尺寸（两者窗口形态相同，差别只在 CSS 是否滑出）；整窗隐藏 → `hideApp()`。ref、React 状态、窗口三者只在 `commit()` 一处一起改，杜绝错配。

所有触发源都只是它的调用方：鼠标感应、打开 / 关闭面板、托盘显示隐藏、挂件右键、`passthrough-state` 广播、`collapse-panel`（全屏）、拖动结束、覆盖层面板按钮。四条关键规则：

- **动画只属于「面板收起」**：只有鼠标离开自动收起与点叉关闭传 `animate: true`（延迟卸载才能播 CSS 滑出）。托盘显隐、穿透、全屏一律立即落定——它们没有可视动画，延迟只会制造「界面已变、窗口没变」的空窗，而这段空窗里 `proximity` 还会把已决定的收起取消掉。
- **`appHidden` 立即写入 ref**（不等 commit）：`proximity` 依据它早退，否则在动画与异步窗口动作的空窗里仍会响应鼠标。
